...
// Your C++-specific code here
    ...
    uint64_t result = largest_power_of_two_less_than(ratio);
        if (DEBUG) printf("Largest power of 2 less than %llu is %llu\n", ratio, result);
    ...
    // Allocate the buffer 
	if (DEBUG) printf("allocating %llu bytes for buffer\n",buffer_size*sizeof(MemoRecord));   
    ...
    if (buffer == NULL) {
    	...
    	if (DEBUG) printf("allocating %llu bytes for bufferShuffled\n",buffer_size*sizeof(MemoRecord));
    ...
    for (unsigned long long i = 0; i < num_buckets; i=i+num_buckets_to_read) 
    {
        ...
        for (unsigned long long r = 0; r < rounds; r++) 
    	{
    		...
    		if (DEBUG) printf("read data: offset_src=%llu bytes=%llu\n",
        	...
        	if (DEBUG) printf("storing read data at index %llu\n",index);
